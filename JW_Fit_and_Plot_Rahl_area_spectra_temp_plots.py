#Fits the D1, D3, and G peak using the method descirbed in the Rahl paper. 
# Makes scatter and spectrum plots of the evolution oof the ceenter point of irradation. 
...
#  y = fname_split[fname_split.index('Y') + 1]
  map = fname_split[fname_split.index('Y--40') + 1]
...
datadir = '/Users/amylebleu-debartola/Desktop/Raman_Damage/Jbilet_Winselwan/focus/ctr_damage' #directory where data is 
AllFiles = np.array([])                   #declare empty array to be filled
for file in glob.glob(os.path.join(datadir, 'Jbilet_Winselwan1_*.txt')):       #start loop to go to the directory above, get all files with .txt
    print(file)
...
for j in range (k):  #the range of 0 to the number of files
   ...
   # print(parse_one)
    ...
    if laser_time_str == 's10x4':
      laser_time = 40
    elif laser_time_str == '8s9x':
      laser_time = 72
    else:
      raise ValueError("Label for time on file name not valid!")
    ...
    def trip_lorentzian_extended(x, *params):
        """Combined double Lorentzianw/ polynomial etc"""
      # Destructure and assign parameters passed in
        const, y1, y2, amp_d1, ctr_d1, wid_d1, amp_d3, ctr_d3, wid_d3, amp_g, ctr_g, wid_g = params
      # Calculate our double Lorentzian with the baseline added
        return (lorentzian(x, amp_d1, ctr_d1, wid_d1)
                + lorentzian(x, amp_d3, ctr_d3, wid_d3)
                + lorentzian(x, amp_g, ctr_g, wid_g)
                + const + (y1 * x) + (y2 * np.power(x, 2.)))

    # Set initial parameters and bounds
    # -> All values in this order:
    #                 const, y1,   y2,  amp_d1, ctr_d1, wid_d1,amp_d3, ctr_d3, wid_d3, amp_g, ctr_g, wid_g 
    initial_params = [0.,    0.45, 0.2,    2e4,   1350.,  150.,   1e4,    1510., 74.,    1e4,  1600.,  30.] 
    lower_bounds = [-1000.,  -1.5, 0.,    0.01,   1330.,  68.,    0.,     1509., 73.,     0.,  1580.,  10.] 
    upper_bounds = [1500.,   1.8,  1.4,    5e4,   1380.,  240.,   5e4,    1511., 75.,   5e4,  1620.,  75.]
    ...
    fitted_y = trip_lorentzian_extended(x, *fitted_params)

    #calculate error array
    error_cords = np.sqrt(np.diag(pcov))
    wid_d1_error = error_cords[5]
    wid_d3_error = error_cords[8]
    wid_g_error = error_cords[11]
       
    ctr_d1_error = error_cords[4]
    ctr_d3_error= error_cords[7]   
    ctr_g_error= error_cords[10]  
...
#    print(error.shape)
    ...
    y1 =fitted_params[1]
    y2= fitted_params[2]
    amp_d1 =fitted_params[3]
    ctr_d1 =fitted_params[4]
    wid_d1 =fitted_params[5]
    amp_d3 =fitted_params[6]
    ctr_d3 =fitted_params[7]
    wid_d3 =fitted_params[8]
    amp_g =fitted_params[9]
    ctr_g =fitted_params[10]
    wid_g =fitted_params[11]

    #calculate the fitted y and area for each peak
    interval_for_simps = (x[1]-x[0])

    peak1 = lorentzian(x,amp_d1, ctr_d1, wid_d1 )
    area_d1_sims =simps(peak1, dx=interval_for_simps)
    true_area_d1 = area_d1_sims*high_y

    peak2 = lorentzian(x,amp_d3, ctr_d3, wid_d3 )
    area_d3_sims =simps(peak2, dx=interval_for_simps)
    true_area_d3 = area_d3_sims*high_y

    peak3= lorentzian(x,amp_g, ctr_g, wid_g )
    area_g_sims =simps(peak3, dx=interval_for_simps)
   ...
   # plot_fitted_data(x, y_no_base, fitted_no_base, peak1, peak2,baseline, extra )    
    ...
    #calcalute temp based on the three peak fit Rahl paper 
    fwhm = wid_d1
        
    r2 = true_area_d1/(true_area_g+true_area_d1)
    r1 = amp_d1/amp_g
    temp2= 737.3+(320.9*r1)-(1067*r2)-(80.638*(r1**2))
    #placeholder error, esimtates about +- 16-50C
    temp_error = 16 
    ...
    Fit_params = (const,y1,y2, amp_d1,ctr_d1,wid_d1,amp_g,ctr_g,wid_g, map_for_csv,power, high_y, temp2,temp_error, wid_d1_error, ctr_d1_error,wid_g_error, ctr_g_error, laser_time, true_area_d1, true_area_g)
...
while k < Fit_param_size:
      ...
      elif this_power == '5-0pct' or this_power == '5pct':
        power_float = 5.0
        p_5_c.append(this_c)
      ...
      elif this_power == '10pct' or this_power == '10-0pct':       
        power_float = 10.0
      ...
      if this_power == '50pct':
        temp_blow50.append(np.nan)
      else:
        temp_blow50.append(this_temp_sort)
...
##Need both of above to fill
    
power_legend = ['all','0.1%', '0.5%', '1%','5%','10%',]
...
#baseline C vs map_number, colors coresponding to dose power 
plt.clf
plt.figure()
ax = plt.gca()
...
plt.gca().legend(power_legend)
plt.title('Baseline Constant vs Trial of '+name_plots)
plt.xlabel('Trial number')
...
np.savetxt(name+"_2lor_Rahl.csv", data_fit_all, delimiter=",", fmt='%s')
