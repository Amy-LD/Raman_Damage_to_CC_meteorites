#Fits  D and G band.
#Busemann equation model 
#creats several spectra and scatter plots 
...
#    print(error.shape)
   ...
   # plot_fitted_data(x, y_no_base, fitted_no_base, peak1, peak2,baseline, extra )    
    ...
    fwhm = wid_d*2
    temp2 = 931-(5.1*fwhm)+(0.0091*(fwhm*fwhm))
    temp_error =  (((wid_d_error)/(wid_d*2)*(5.1*fwhm))**2 + ((2*(0.0091*(fwhm*fwhm)))*((wid_d_error)/(wid_d*2)))**2)**0.5
...
np.savetxt(name+"_2lor_Bus.csv", data_fit_all, delimiter=",", fmt='%s')
