##Makes a heat map of the area of each point of the D anc G carbon bands. 
#Non-normlaized fit
# Fits 2 lorenzians to data, adds area, then makes a seaborn and custom heatmap. 
...
for j in range(k):
   ...
   # print(parse_one)
    ...
    area_d_sims =simps(peak1, dx=5) * high_y
    #print(area_d_sims)

    peak2 = lorinzian(x,amp_g_2p, ctr_g_2p, wid_g_2p )
    area_g_sims =simps(peak2, dx=5) * high_y

    area_t = area_d_sims+area_g_sims
   ...
   #print("norm factor", high_y)
    ...
    if np.float(amp_d_2p) < 0.01 and np.float(amp_g_2p) < 0.01 and ratio >2.5 :
      ...
      carbon_amount = np.append(carbon_amount, area_t)
...
np.savetxt(name+"_2lor_Schmidt.csv", data_fit_all, delimiter=",")
...
for idx, carbon in enumerate(carbon_amount):
  x_idx = sorted_x.index(cords_x[idx])
  y_idx = sorted_y.index(cords_y[idx])
  arranged_data[(y_idx, x_idx)] = carbon
...
plt.title('Total Carbon Area Pre-Heating')
plt.xlabel('x-axis')
plt.ylabel('y-axis')

sns.heatmap(arranged_data, vmin=0, vmax=100000000, alpha = 1, cmap=sns.color_palette('Spectral_r', 100), ax=ax, zorder = 1)
...
plt.savefig(name+"_2lor_C_amount.png")
plt.show()
